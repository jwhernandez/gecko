	...
	$.fn.paginate = function( options ) {
		...
		function createPaginationControls( pages ) {
			...
			if ( options.prev_next == true ) {
				obj.find( "." + options.pagination_class ).append( '<li class="prev pag"><a><h4>' + options.prev_text + '</h4></a></li>' );
			}
			// For each div (slide) add a link in span for controls
			for ( var i = 0; i < pages; i++ ) {
				obj.find( "." + options.pagination_class ).append( '<li class="pag"><a href="#' + ( i + 1 ) + '"><h4><strong>' + ( i + 1 ) + '</strong></h4></a></li>' );
			};

			// Add Next Button
			if ( options.prev_next == true ) {
				obj.find( "." + options.pagination_class ).append( '<li class="next pag"><a><h4>' + options.next_text + '</h4></a></li>' );
			}
		}
